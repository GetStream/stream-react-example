...
import { PhotoList, Avatar } from 'components';
...
class Landing extends Component {
     ...
     */
	componentDidMount() {
		findDOMNode(this._pageElm).addEventListener(
			'scroll',
			this.handleScroll,
		);
	}
     ...
     */
	componentWillUnmount() {
		findDOMNode(this._pageElm).removeEventListener(
			'scroll',
			this.handleScroll,
		);
	}
		...
		return (
			<div>
				<PhotoList
					...
					photos={this.props.photos.filter(p => !p.hidden)}
				/>
...
class Home extends Component {
	...
	handleLike = data => {
		this.props.dispatch(
			PhotosActions[data.liked ? 'like' : 'unlike'](data.id),
		);
	};
	...
	render() {
		return (
			...
			<Landing
				...
				onLoadHidden={this.handleLoadHidden}
